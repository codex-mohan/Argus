...
/* ΓöÇΓöÇΓöÇ Agent Data ΓöÇΓöÇΓöÇ */
const agents = [
  {
    name: "MarketDataBot",
    color: "#4ADE80",
  ...
  },
  {
    ...
    frequency:
      "Continuous ΓÇö every 15 minutes for breaking news, hourly for sentiment",
  },
];

/* ΓöÇΓöÇΓöÇ Lens Data ΓöÇΓöÇΓöÇ */
const lenses = [
  {
    name: "GTM Lens",
    color: "#C4973B",
...
/* ΓöÇΓöÇΓöÇ API Endpoints ΓöÇΓöÇΓöÇ */
const endpoints = [
  {
    method: "GET",
    path: "/health",
...
/* ΓöÇΓöÇΓöÇ Environment Variables ΓöÇΓöÇΓöÇ */
const envVars = [
  {
...
/* ΓöÇΓöÇΓöÇ Code Block Component ΓöÇΓöÇΓöÇ */
function CodeBlock({
  code,
  language = "bash",
}: {
...
/* ΓöÇΓöÇΓöÇ Section Heading ΓöÇΓöÇΓöÇ */
...
/* ΓöÇΓöÇΓöÇ Inline Badge ΓöÇΓöÇΓöÇ */
function Badge({
  children,
  color = "#4ADE80",
}: {
...
/* ΓöÇΓöÇΓöÇ Method Badge ΓöÇΓöÇΓöÇ */
...
/* ΓöÇΓöÇΓöÇ Main Docs Page ΓöÇΓöÇΓöÇ */
export default function DocsPage() {
  ...
  return (
    ...
    >
      ...
      >
        {/* ΓòÉΓòÉΓòÉ LEFT SIDEBAR ΓòÉΓòÉΓòÉ */}
        <aside
          style={{
            width: "260px",
            flexShrink: 0,
        ...
        >
          ...
          <p style={pStyle}>
            ...
            specialized agents and analyzes it through three distinct lenses ΓÇö
            Go-to-Market, Finance, and Security ΓÇö connected by a persistent
            knowledge graph.
          </p>
          <div
            style={{
          ...
          <ul style={ulStyle}>
            <li>
              <span style={{ color: "#F5F5F0" }}>Node.js 20+</span> ΓÇö runtime
              for the agent backend and Next.js frontend
            </li>
            <li>
              <span style={{ color: "#F5F5F0" }}>Bun</span> ΓÇö package manager
              and build tool
            </li>
            <li>
              ...
              ΓÇö required for Cognee MCP service
            </li>
            <li>
              <span style={{ color: "#F5F5F0" }}>Bright Data API key</span> ΓÇö
              web scraping and structured extractors
            </li>
            <li>
              <span style={{ color: "#F5F5F0" }}>AI/ML API key</span> ΓÇö LLM
...
# Terminal 1 ΓÇö Backend agent server
cd apps/backend && bun run dev

# Terminal 2 ΓÇö Frontend dashboard
...
curl http://localhost:3001/health`}
          ...
          >
            <pre style={{ margin: 0 }}>
              {`  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ DATA COLLECTION ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
  Γöé                                                        Γöé
  Γöé  MarketDataBot  FilingDataBot  SocialDataBot           Γöé
  Γöé  SupplierDataBot  NewsDataBot                          Γöé
  Γöé                                                        Γöé
  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
                          Γöé
                          Γû╝
  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ COGNEE MEMORY ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
  Γöé                                                       Γöé
  Γöé  `}
              ...
              {`   Γöé
  Γöé  Persistent knowledge graph ┬╖ Cross-referenced        Γöé
  Γöé                                                       Γöé
  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
                          Γöé
                          Γû╝
  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ ANALYSIS LENSES ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
  Γöé                                                       Γöé
  Γöé  `}
  ...
  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
                          Γöé
                          Γû╝
  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ SYNTHESIS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
  Γöé                                                       Γöé
  Γöé  CorrelationEngine ΓåÆ BriefWriter ΓåÆ Output Briefs      Γöé
  Γöé                                                       Γöé
  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ`}
              ...
              <tbody>
                {[
                  ...
                  [
                    ...
                    "400+ models ΓÇö DeepSeek for reasoning, Claude for reports, Gemini for ingest",
                  ],
                  [
                    "Web Data",
                    "Bright Data SDK + MCP",
          ...
          {/* ΓöÇΓöÇ COLLECTION AGENTS ΓöÇΓöÇ */}
  ...
  source_url: string;        // Required ΓÇö the URL this data came from
  scraped_at: string;        // Required ΓÇö ISO 8601 timestamp
  agent_id: string;          // Required ΓÇö which agent stored this
  confidence: number;        // Required ΓÇö 0.0-1.0 (< 0.7 = low_confidence)
  data_type: string;         // Required ΓÇö "price" | "filing" | "social" | "supplier" | "news"
  content: string;           // Required ΓÇö the actual finding text
  raw_extract?: object;      // Optional ΓÇö original structured data
  ttl_hours?: number;        // Optional ΓÇö staleness threshold
}`}
            language="typescript"
          />
...
export const config = {
  ...
  retries: 3,
  backoff: "exponential",            // 1s ΓåÆ 2s ΓåÆ 4s
  ...
  cognee_data:`}
          ...
          <ul style={ulStyle}>
            ...
            <li>
              ...
              Docker memory limits ΓÇö backend agents can be memory-intensive
...
/* ΓöÇΓöÇΓöÇ Shared Inline Styles ΓöÇΓöÇΓöÇ */
